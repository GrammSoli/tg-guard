      ...
      <header className="flex items-center justify-between px-5 pt-6 pb-4">
        ...
        <button
          onClick={() => {
            hapticImpact("medium");
            if (
              paywallConfig.paywall_enabled &&
              !isSubscribed &&
              rooms.length >= paywallConfig.free_room_limit
            ) {
              setPremiumOpen(true);
      ...
      <section className="space-y-2 px-5">
        {rooms.length === 0 ? (
          <div className="bg-surface flex flex-col items-center gap-3 rounded-xl border border-white/10 p-8 text-center">
            ...
            <p className="text-xs text-muted-foreground">{t("rooms.noRoomsHint")}</p>
            <button
          ...
          <div className="bg-surface flex flex-col items-center gap-2 rounded-xl border border-white/10 p-6 text-center">
            ...
            <p className="text-xs text-muted-foreground">{t("rooms.nothingFoundHint")}</p>
          </div>
        ) : (
          filteredRooms.map((room) => {
            const rawAmount =
              typeof room.total_per_member === "number" && isFinite(room.total_per_member)
