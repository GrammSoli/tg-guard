...
func handleSuccessfulPayment(ctx context.Context, b *tgbot.Bot, update *models.Update, db *gorm.DB) {
	...
	// Format: "premium_stars_<plan>_<userID>" — exactly 4 parts. The
	// legacy 3-part "premium_stars_<userID>" used to be coerced to a
	// LIFETIME grant; removed under audit Tier-1 #2 so a forged or
	// stale 3-part payload can no longer claim the most expensive
	// tier. Stars invoices expire in hours, so any in-flight legacy
	// invoice is gone by now; users with a fresh purchase get the
	// 4-part payload generated by handler/payment.go.
	...
	if len(parts) != 4 {
		log.Printf("❌ [bot/payment] Malformed payload=%q (got %d parts, want 4)", payment.InvoicePayload, len(parts))
		return
	}
	// premium / stars / plan / uid
	...
	// ── Step 2: Idempotency via INSERT … ON CONFLICT DO NOTHING ──
	// Was a pre-check SELECT + tx with INSERT inside. The SELECT/INSERT
	// pair had a TOCTOU window where two concurrent SuccessfulPayment
	// deliveries (Telegram retries the webhook on slow/non-200 response)
	// both passed the SELECT and then raced on the UNIQUE constraint
	// inside their respective tx — the loser tx rolled back with an
	// error log, generating Sentry noise for what was functionally a
	// no-op. The atomic insert-or-skip below collapses that into one
	// statement: first writer wins and flips premium in the same tx;
	// every subsequent caller sees RowsAffected == 0 and returns
	// silently. Mirrors handler/payment.go HandleCryptoWebhook.
	// Audit Tier-1 #1.
	...
	// ── Step 3: Find user — needed for donation row + congrat message ──
	var user model.User
	...
	// ── Step 4: DB update ───────────────────────────────────────
	log.Printf("💽 [bot/payment] Attempting to activate premium for user %d", user.ID)
	...
	txErr := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if result.RowsAffected == 0 {
			...
			// the user.is_donator flip was committed then.
			return nil
		}
		firstTime = true
	...
	if !firstTime {
		log.Printf("⚠️ [bot/payment] Duplicate charge=%s for user=%d — skipping congrat", chargeID, user.ID)
		return
	}
